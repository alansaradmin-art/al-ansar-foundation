...
export default async function handler(req: ApiRequest, res: ApiResponse) {
  ...
  // Folded into this file rather than its own api/audit-logs.ts — stays
  // under Vercel's Hobby-plan 12-serverless-function-per-deployment cap
  // (main was already sitting at exactly 12). Both are Admin-oriented
  // oversight/configuration concerns, called before any settings-specific
  // query param is read.
  if (readQueryParam(req, 'resource') === 'auditLogs') return handleAuditLogs(req, res, profile, supabase)
...
async function handleAuditLogs(req: ApiRequest, res: ApiResponse, profile: CallerProfile, supabase: Supabase) {
  ...
  // ownership is verified the same way api/members.ts's ?resource=documents
  // does it.
  ...
  try {
    // GET ?resource=auditLogs&action=actors — the data source for the
    ...
    console.error('[api/settings?resource=auditLogs]', error)
    sendError(res, 500, 'Something went wrong. Please try again.')
  }
}
