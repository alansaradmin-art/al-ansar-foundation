-- Add mobile_number to the Donation Engagement Report so Admin can follow
-- up directly from the report, matching every other member-listing report
-- in this app. Changes the function's return column list with the same
-- argument list, which `create or replace function` cannot do — drop and
-- recreate instead (same reasoning as 0017). Nothing else references this
-- function, so the drop is safe.
