/** ISO 3166-1 alpha-2 code + English short name + E.164 calling code, for
 * every country/territory with its own dial code. This is the single
 * source of truth for the country picker (CountryPhoneField) and for
 * deriving a full international number from a stored (country, local
 * number) pair — see buildInternationalNumber below, the deterministic
 * replacement for format.ts's old length-guessing toWhatsAppNumber.
 *
 * A few dial codes are shared by multiple countries (e.g. +1 for both US
 * and Canada) — that's fine here since the country itself, not the dial
 * code, is what's stored; the dial code is only ever derived forward from
 * a known country, never guessed backward from a number's length. */
...
/** ISO2 -> flag emoji, via the regional-indicator-symbol trick (each letter
 * maps to its own Unicode "regional indicator" codepoint; the OS/font
 * renders the pair as a flag). No per-country emoji needs to be stored. */
export function countryFlag(iso2: string): string {
  return String.fromCodePoint(...[...iso2.toUpperCase()].map((c) => 127397 + c.charCodeAt(0)))
}

/** "India (+91)" — used as both the picker's selected-value label and the
 * search-result row label. */
export function formatCountryLabel(country: Country): string {
  return `${country.name} (+${country.dialCode})`
}

/** The deterministic replacement for format.ts's old length-guessing
 * toWhatsAppNumber: dial code + digits-only local number, straight from a
 * known (country, local number) pair — no ambiguity, since the country
 * was chosen explicitly rather than inferred from digit count. Returns
 * null when the country isn't recognized or the number is empty, so
 * callers can fall back to the legacy guess for un-migrated rows. */
export function buildInternationalNumber(iso2: string | null | undefined, localNumber: string | null | undefined): string | null {
